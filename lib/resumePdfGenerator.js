    ...
    try {
      ...
      const docDefinition = {
        ...
        content: [
          ...
          // ── WHITE NAME BOX (centred overlay) ──────────────────────────
          {
            margin: [60, -75, 60, 20],
            table: {
              widths: ["*"],
                ...
                [
                  {
                    stack: [
                      {
                        text: safe(data.fullName).toUpperCase() || "YOUR NAME",
                        alignment: "center",
                        fontSize: 22,
                        bold: true,
                        margin: [0, 8, 0, 4],
                      ...
                      {
                        text: spaced(
                          (safe(data.designation) || "YOUR ROLE").toUpperCase()
                        ),
                        alignment: "center",
                        fontSize: 10,
                        color: "#777",
            ...
            columns: [
                ...
                stack: [
                  ...
                  sectionHalf("CONTACT"),
                  {
                    stack: [
                      {
                        columns: [
                          { text: "Phone:", bold: true, width: "auto", fontSize: 9 },
                          { text: "  " + (safe(data.phone) || "-"), fontSize: 9 },
                        ],
                        margin: [0, 0, 0, 4],
                      },
                      {
                        columns: [
                          { text: "Email:", bold: true, width: "auto", fontSize: 9 },
                          { text: "  " + (safe(data.email) || "-"), fontSize: 9 },
                        ],
                        margin: [0, 0, 0, 4],
                      },
                      {
                        columns: [
                          { text: "LinkedIn:", bold: true, width: "auto", fontSize: 9 },
                          {
                            text: "  " + (safe(data.linkedin) || "-"),
                            fontSize: 9,
                          },
                        ],
                        margin: [0, 0, 0, 4],
                  ...
                  sectionHalf("SKILLS"),
                  {
                    text: arr(data.skills).length
                      ? arr(data.skills).join("\n")
                      : "—",
                    fontSize: 9,
                    lineHeight: 1.4,
                    margin: [0, 0, 0, 4],
              ...
              // ── RIGHT COLUMN ─────────────────────────────────────────
              {
                ...
                stack: [
                  ...
                  ...(arr(data.experience).length
                    ? arr(data.experience).map((exp) => ({
                        stack: [
                          ...
                          {
                            text: `${safe(exp.startDate) || "-"}${
                              exp.endDate ? " – " + safe(exp.endDate) : " – Present"
                            }`,
                            fontSize: 8,
                            color: "#777",
                            margin: [0, 1, 0, 2],
                          ...
                          ...(Array.isArray(exp.bullets) && exp.bullets.length
                            ...
                            : exp.description
                            ? [{ ul: [safe(exp.description)], fontSize: 9 }]
                            : []),
                        ],
                        margin: [0, 4, 0, 0],
                      }))
                    ...
                    ? arr(data.projects).map((proj) => ({
                        stack: [
                          ...
                          ...(Array.isArray(proj.bullets) && proj.bullets.length
                            ...
                            : proj.description
                            ? [{ ul: [safe(proj.description)], fontSize: 9 }]
                            : []),
                        ],
                        margin: [0, 4, 0, 0],
                      }))
                  ...
                  sectionHalf("TOOLS"),
                  {
                    ul: arr(data.tools).length ? arr(data.tools) : ["—"],
                    fontSize: 9,
                    margin: [0, 0, 0, 4],
                  },
